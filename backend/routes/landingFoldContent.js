...
const landingFoldRouter = express.Router();
const db = require('../db');
const { orgCode } = require('../utilityFunctions');

landingFoldRouter.get("/landingfold", (req, res) => {
    try {
        db.query(`SELECT * FROM landing WHERE isActive=1 AND orgCode='${orgCode}';`, (err, result) => {
            if (err) {
...
module.exports = landingFoldRouter;