            ...
            if (serviceParsedResponse['result']['status']['statusCode'] == "0") {
                ...
                for (let i = 0; i < serviceDetails.length; i++) {
                    serviceParsedResponseInfo += `<tr>
                        <td>${i + 1}</td>
                        <td><i class="${serviceDetails[i].icon}"></i></td>
        ...
        success: (response) => {
            ...
            if (editServiceParsedResponse['result']['status']['statusCode'] == "0") {
                ...
                if (serviceDetail.length > 0) {
                    ...
                    $("#showEditIcons").html(`<i class="${serviceDetail[0]['icon']}"></i> ${serviceDetail[0]['icon']}`);
                    $("#editIcons").val(serviceDetail[0]['icon']);
...
$(document).ready(() => {
    ...
    // Get the input element
    const bannerImageInput = document.getElementById('addBannerImageInput');

    // Get the preview BannerImage element
    ...
    bannerImageInput.addEventListener('change', function (event) {
        ...
        if (event.target.files && event.target.files[0]) {
            ...
            // Set the BannerImage source once it's loaded
            reader.onload = function (e) {
    ...
    // Get the input element
    const bannerImageEditInput = document.getElementById('editBannerImageInput');

    // Get the preview BannerImage element
    const previewEditBannerImage = document.getElementById('previewEditBannerImage');

    // Get the Stored BannerImage element
    const storedViewBannerImage = document.getElementById('storedViewBannerImage');

    // Listen for changes in the input file selection
    bannerImageEditInput.addEventListener('change', function (event) {
        // Check if a file is selected
        if (event.target.files && event.target.files[0]) {
            // Create a FileReader object
            const reader = new FileReader();
            ...
            // Set the BannerImage source once it's loaded
            reader.onload = function (e) {
                previewEditBannerImage.src = e.target.result;
                previewEditBannerImage.style.display = 'block';
                storedViewBannerImage.style.display = 'none';
            };
            ...
            // No file selected, hide the preview
            previewEditBannerImage.style.display = 'none';
            storedViewBannerImage.style.display = 'block';
        }
    });
