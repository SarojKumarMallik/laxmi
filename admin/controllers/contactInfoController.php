<?php
	...
	function checkExistsOrNot($conn, $contactId) {
        $sql = "SELECT COUNT(id) AS COUNTS FROM contactinfo WHERE id = '$contactId' AND orgCode = '".$_SESSION['orgCode']."'";
	...
	try {
        ...
        if ($checkExistsOrNot == 0) {
            ...
            $sql = "UPDATE `contactinfo` SET `shortText` = '$shortText', `mapIframe` = '$mapIframe', `other` = '$otherLink', `address` = '$address', `email` = '$email', `phone` = '$call', `isActive` = '$status', `social_links` = '$socialLinks', `updated` = NOW() WHERE id = '$contactId' AND orgCode = '".$_SESSION['orgCode']."'";
        }
		$result = mysqli_query($conn, $sql);
