<?php
	...
	try {
		...
		$title = trim($_POST['addMainMenuTitleInput']);
		$slNo = trim($_POST['addMainMenuSlNo']);
		$slug = $_POST['addSlug'];
		...
		$menuSql = "INSERT INTO `menus` (`title`, `slug`, `sl_no`, `created` `orgCode`) VALUES ('$title', '$slug', '$slNo', NOW(), '".$_SESSION['orgCode']."');";
		...
		if ($menuResult) {
			...
			if (count($submenuMenuItems) > 0) {
				for ($i=0; $i < count($submenuMenuItems); $i++) {
                    ...
                    $submenuSql = "INSERT INTO `sub_menus` (`mainmenu_id`, `title`, `slug`, `created`, `orgCode`) VALUES ('$lastInsertedId', '$title', '$slug', NOW(), '".$_SESSION['orgCode']."')";
