<?php
	...
	function checkSlNoExistsOrNot($conn, $slNo){
		$sql = "SELECT COUNT(id) AS COUNTS FROM footer_menus WHERE sl_no = '$slNo' AND orgCode = '".$_SESSION['orgCode']."'";
	...
	try {
		...
		$existingSubMenuIds = [];
		$getSubmenuSql = mysqli_query($conn, "SELECT id FROM `footer_sub_menus` WHERE footermenu_id='$footerMenuId' AND deleted IS NULL AND orgCode = '".$_SESSION['orgCode']."'");
		if ($getSubmenuSql) {
		...
		if ($checkSlNoExistsOrNot > 0) {
			$sql = "UPDATE `footer_menus` SET  `title` = '$title', `updated` = NOW() WHERE id = '$footerMenuId' AND orgCode = '".$_SESSION['orgCode']."'";
		}else {
        	$sql = "UPDATE `footer_menus` SET  `title` = '$title', `sl_no` = '$slNo', `updated` = NOW() WHERE id = '$footerMenuId' AND orgCode = '".$_SESSION['orgCode']."'";
		}
		$result = mysqli_query($conn, $sql);
