<?php
	...
	function checkExistsOrNot($conn, $disclaimerId) {
        $sql = "SELECT COUNT(id) AS COUNTS FROM disclaimer WHERE id = '$disclaimerId' AND orgCode = '".$_SESSION['orgCode']."'";
	...
	try {
        ...
        if ($checkExistsOrNot == 0) {
            ...
            $sql = "UPDATE `disclaimer` SET `privacy_policy` = '$privacyPolicyInput', `terms_n_conditions` = '$termsAndConditionInput', `updated` = NOW() WHERE id = '$disclaimerId' AND orgCode = '".$_SESSION['orgCode']."'";
        }
