<?php
	...
	function checkSlNoExistsOrNot($conn, $slNo){
		$sql = "SELECT COUNT(id) AS COUNTS FROM menus WHERE sl_no = '$slNo' AND orgCode = '".$_SESSION['orgCode']."'";
	...
	try {
		...
		$existingSubMenuIds = [];
		$getSubmenuSql = mysqli_query($conn, "SELECT id FROM `sub_menus` WHERE mainmenu_id='$mainMenuId' AND deleted IS NULL AND orgCode = '".$_SESSION['orgCode']."'");
		if ($getSubmenuSql) {
		...
		if ($checkSlNoExistsOrNot > 0) {
			$sql = "UPDATE `menus` SET  `title` = '$title', `slug` = '$slug', `updated` = NOW() WHERE id = '$mainMenuId' AND orgCode = '".$_SESSION['orgCode']."'";
		} else {
        	$sql = "UPDATE `menus` SET  `title` = '$title', `slug` = '$slug', `sl_no` = '$slNo', `updated` = NOW() WHERE id = '$mainMenuId' AND orgCode = '".$_SESSION['orgCode']."'";
		}
		$result = mysqli_query($conn, $sql);
