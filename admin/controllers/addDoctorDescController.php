<?php
	...
	function checkExistsOrNot($conn, $editDoctorDescId) {
        $sql = "SELECT COUNT(id) AS COUNTS FROM doctor_page_description WHERE id = '$editDoctorDescId' AND orgCode = '".$_SESSION['orgCode']."'";
	...
	try {
        ...
        if ($checkExistsOrNot == 0) {
            $sql = "INSERT INTO `doctor_page_description` (`content`, `isActive`, `created`, `orgCode`) VALUES ('$addDesc', '$aboutStatus', NOW(), '".$_SESSION['orgCode']."')";
                ...
                $sql = "UPDATE `doctor_page_description` SET `content` = '$addDesc', `isActive` = '$aboutStatus', `updated` = NOW() WHERE id = '$editDoctorDescId' AND orgCode = '".$_SESSION['orgCode']."'";
        }
		$result = mysqli_query($conn, $sql);
