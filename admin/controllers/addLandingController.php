<?php
	...
	function checkExistsOrNot($conn, $landingId) {
        $sql = "SELECT COUNT(id) AS COUNTS FROM landing WHERE id = '$landingId' AND orgCode = '".$_SESSION['orgCode']."'";
	...
	try {
        ...
        if ($checkExistsOrNot == 0) {
			...
			if ($imageFile['name'] != "" && $bgImageFile['name'] != "") {
				...
				$sql = "UPDATE `landing` SET `organization_name` = '$orgName', `title` = '$title', `description` = '$desc', `link` = '$link', `linkText` = '$text', `bg_img` = '$bgImageName', `logo` = '$imageName', `isActive` = '$aboutStatus', `updated` = NOW() WHERE id = '$landingId' AND orgCode = '".$_SESSION['orgCode']."'";
			} else if ($bgImageFile['name'] != "") {
				// echo "bg";
				$bgImageName = str_replace("'", "\'",handleFile($bgImageFile, "../../img/uplaods/"));
				$sql = "UPDATE `landing` SET `organization_name` = '$orgName', `title` = '$title', `description` = '$desc', `link` = '$link', `linkText` = '$text', `bg_img` = '$bgImageName', `isActive` = '$aboutStatus', `updated` = NOW() WHERE id = '$landingId' AND orgCode = '".$_SESSION['orgCode']."'";
			} else if ($imageFile['name'] != "" ) {
				// echo "logo";
				$imageName = str_replace("'", "\'", handleFile($imageFile, "../../img/uplaods/"));
				$sql = "UPDATE `landing` SET `organization_name` = '$orgName', `title` = '$title', `description` = '$desc', `link` = '$link', `linkText` = '$text', `logo` = '$imageName', `isActive` = '$aboutStatus', `updated` = NOW() WHERE id = '$landingId' AND orgCode = '".$_SESSION['orgCode']."'";
			} else {
                $sql = "UPDATE `landing` SET `organization_name` = '$orgName', `title` = '$title', `description` = '$desc', `link` = '$link', `linkText` = '$text', `isActive` = '$aboutStatus', `updated` = NOW() WHERE id = '$landingId' AND orgCode = '".$_SESSION['orgCode']."'";
			}
        }
		$result = mysqli_query($conn, $sql);
