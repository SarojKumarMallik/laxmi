...
require '../assets/includes/checkLogin.php';
	...
	function checkExistsOrNot($conn, $aboutId) {
        $sql = "SELECT COUNT(id) AS COUNTS FROM about WHERE id = '$aboutId' AND orgCode = '".$_SESSION['orgCode']."'";
	...
	try {
        ...
        if ($checkExistsOrNot == 0) {
            ...
            $sql = "UPDATE `about` SET `heading` = '$headingOne', `description` = '$DescOne', `videoLink` = '$videoLink', `videoIframe` = '$videoFrame', ".$imgAddon." `more` = '$DescTwo', `moreHeading` = '$headingTwo', `isActive` = '$aboutStatus', `updated` = NOW()  WHERE id = '$aboutId' AND orgCode = '".$_SESSION['orgCode']."'";
        }
